...
import BusinessInfoCard from '../../../components/Cultivator/WeedProductCard/WeedProductCard';
...
class BusinessInfoCards extends Component {
    ...
    componentDidMount () {
        ...
        axios.get( 'https://mit-location-svc.mybluemix.net/api/v1/near?long=-118.291209&lat=33.957239&radius=100000' )
            .then( res => {
                ...
                this.setState( { businesss: res.data.locations } );
      ...
      render() {
        let Businessinfocard = this.state.business.map(biz => {
            return (
                <Container>
                    <Row>
                <Col xs="6" sm="4">
                <BusinessInfoCard key={biz.id} biz={biz} />
                </Col>
                </Row>
                </Container>
            )
